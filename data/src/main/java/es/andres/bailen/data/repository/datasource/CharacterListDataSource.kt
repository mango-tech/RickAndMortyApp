...
import es.andres.bailen.data.extensions.getQueryParameter
...
class CharacterListDataSource(private val api: RickyMortyApi): PagingSource<Int, CharacterModel>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, CharacterModel> {
        return withContext(Dispatchers.IO) {
            ...
            val prevKey: Int? = pagingModel.info.prev?.getQueryParameter("page")
            val nextKey: Int? = pagingModel.info.next?.getQueryParameter("page")
            LoadResult.Page(
